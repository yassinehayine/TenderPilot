...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb">Workspace <span>/</span> {activePage}</div><div className="top-actions"><button className="icon-button" aria-label="Notifications">◌</button><button className="help-button">? <span>Help center</span></button></div></header><div className="content-wrap">
      ...
      {activePage === 'Upload Tender' ? <UploadTender onUploaded={(tenderId) => { setCurrentTenderId(tenderId); setActivePage('Compliance Matrix'); }} onProcessingFailure={setCurrentTenderId} /> : activePage === 'Compliance Matrix' ? <ComplianceMatrix tenderId={currentTenderId} /> : activePage === 'Go / No-Go' ? <QualificationView tenderId={currentTenderId} /> : activePage === 'Technical Proposal' ? <ProposalView tenderId={currentTenderId} /> : activePage === 'Human Review' ? <ProposalView tenderId={currentTenderId} reviewMode /> : <>
