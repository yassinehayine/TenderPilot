...
export function ProposalView({ tenderId, reviewMode = false }: Props) {
  ...
  async function saveReview(section: ProposalSection, status: 'approved' | 'changes_requested') {
    setBusy(true); setError(null);
    try { const saved = await reviewProposalSection(section.id, status, drafts[section.id]); setSections((current) => current.map((item) => item.id === saved.id ? saved : item)); } catch (reviewError) { setError(reviewError instanceof Error ? reviewError.message : 'Could not save review.'); } finally { setBusy(false); }
  }
