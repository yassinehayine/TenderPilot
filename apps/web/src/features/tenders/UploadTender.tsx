...
export function UploadTender({ onUploaded }: Props) {
  ...
  return <section className="upload-view">
    ...
    {result && <div className={`upload-feedback ${result.processingStatus === 'needs_review' ? 'warning' : 'success'}`}><strong>{result.processingStatus === 'needs_review' ? 'Document needs review' : 'Document ready'}</strong><span>{result.title} · {result.pageCount} page(s)</span>{result.unreadablePages.length > 0 && <span>Unreadable pages: {result.unreadablePages.join(', ')}. No requirements were generated from them.</span>}</div>}
  </section>;
}