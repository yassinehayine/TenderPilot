...
function statusFor(stage: string, current: TenderStatus) {
  const currentStage = current.processingStage === 'extracting_pdf' || current.processingStage === 'extracting_requirements' ? 'extract' : current.processingStage;
  if (stage !== currentStage) return { label: 'Not executed', className: 'pending' };
...
export function AgentExecutionPanel({ tenderId }: Props) {
  ...
  const currentStage = status.processingStage === 'extracting_pdf' || status.processingStage === 'extracting_requirements' ? 'extract' : status.processingStage;
  return <section className="agent-panel" aria-label="Agent execution status"><div className="agent-panel-heading"><div><p className="eyebrow">Agent execution</p><h2>{status.title}</h2></div><span className={`agent-overall ${status.processingStatus}`}>{status.processingStatus.replace('_', ' ')}</span></div><div className="agent-stage-list">{stages.map(([key, label]) => { const item = statusFor(key, status); return <div className={`agent-stage ${item.className}`} key={key}><span className="agent-stage-marker">{item.className === 'complete' ? '✓' : item.className === 'blocked' ? '!' : item.className === 'active' ? '•' : '·'}</span><strong>{label}</strong><span>{item.label}</span>{key === currentStage && <small>Attempt {status.processingAttempt}</small>}</div>; })}</div>{isUnreadable && <p className="agent-escalation"><strong>Human action required:</strong> pages {status.unreadablePages.join(', ')} are unreadable. Requirements were not generated from those pages.</p>}{status.processingError && <p className="agent-escalation"><strong>Processing stopped:</strong> {status.processingError}</p>}</section>;
}